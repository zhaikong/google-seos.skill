...
Google 并不托管任何翻译版网页。通过翻译搜索结果打开网页与通过谷歌翻译打开原始搜索结果或使用 Chrome 浏览器内翻译功能查看网页没有区别。也就是说，网页上的 JavaScript 以及嵌入的图片和其他网页功能通常都受支持。

## 在 Search Console 中监控效果

如需监控翻译搜索结果的点击次数和展示次数，您可以使用效果报告中的搜索结果呈现过滤条件。

## 选择启用或停用翻译搜索结果功能

此功能适用于所有网页和基于用户语言的搜索结果。此功能默认即已启用，您无需进行任何操作。

翻译搜索结果功能与 Google 搜索中的其他翻译相关功能类似。如需选择停用 Google 搜索中的所有翻译功能，请使用 notranslate 规则，该规则可以实现为 meta 标记或 HTTP 标头：
